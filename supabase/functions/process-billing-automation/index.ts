...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    for (const boleto of boletos) {
      try {
        if (!boleto.projeto_id)
          throw new Error('Projeto não encontrado para este boleto.')
        ...
        if (!orcamento)
          throw new Error('Orçamento não encontrado para o projeto do boleto.')
        ...
        if (invalidos.length > 0)
          throw new Error('Existem itens não validados no projeto.')
        ...
        const sumItens = itens.reduce(
          (acc, i) =>
            acc +
            (Number(i.subtotal) ||
              Number(i.quantidade) * Number(i.preco_unitario)),
          0,
        )
