...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (reportType === 'orcamento') {
      ...
      if (logoBase64) {
        try {
          ...
          page.drawImage(image, {
            x: (width - imgWidth) / 2, // Centered perfectly
      ...
      // Header Text
      page.drawText('Luce Nera', { x: 40, y: logoBottomY, size: 14, font: boldFont });
      page.drawText('Manoella Zauith Leite Lopes', { x: 40, y: logoBottomY - 15, size: 9, font });
      page.drawText('14.025-270 Rua Ayrton Roxo 867', { x: 40, y: logoBottomY - 27, size: 9, font });
      page.drawText('Alto Da Boa Vista, Ribeirao Preto/sp', { x: 40, y: logoBottomY - 39, size: 9, font });
      page.drawText('(16) 3442 - 3545', { x: 40, y: logoBottomY - 51, size: 9, font });

      // Approval
      page.drawText('1 de 1', { x: width - 60, y: logoBottomY, size: 9, font: boldFont });
      page.drawLine({ start: { x: width - 200, y: logoBottomY - 20 }, end: { x: width - 40, y: logoBottomY - 20 }, thickness: 1 });
      page.drawText('Aprovação do Cliente', { x: width - 195, y: logoBottomY - 15, size: 8, font });

      page.drawLine({ start: { x: width - 200, y: logoBottomY - 50 }, end: { x: width - 40, y: logoBottomY - 50 }, thickness: 1 });
      page.drawText('Lucenera', { x: width - 195, y: logoBottomY - 45, size: 8, font });

      page.drawText(`Data Impressão ${new Date().toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo' })}`, { x: width - 150, y: logoBottomY - 62, size: 6, font, color: rgb(0.4,0.4,0.4) });
      ...
      page.drawText(fmtSubtotal, { x: width - 100, y: y - 15, size: 10, font });
      
      page.drawText(`Desconto:`, { x: width - 210, y: y - 30, size: 10, font });
      page.drawText(fmtGlobalDesc, { x: width - 100, y: y - 30, size: 10, font });
      
      page.drawText(`Valor Total:`, { x: width - 210, y: y - 48, size: 12, font: boldFont });
      page.drawText(fmtFinalTotal, { x: width - 100, y: y - 48, size: 12, font: boldFont });
