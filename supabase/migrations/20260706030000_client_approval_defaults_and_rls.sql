-- Client Approval: Set default status to enviado_cliente and ensure RLS policies
-- Ensures all approval columns exist (idempotent) and allows authenticated updates

-- Ensure columns exist (idempotent — ADD COLUMN IF NOT EXISTS is safe)
ALTER TABLE public.orcamentos
  ADD COLUMN IF NOT EXISTS enviado_cliente_em timestamptz,
  ADD COLUMN IF NOT EXISTS enviado_cliente_por uuid,
...
-- Set default status to enviado_cliente for new budgets
ALTER TABLE public.orcamentos ALTER COLUMN status SET DEFAULT 'enviado_cliente';

-- RLS: allow authenticated users to insert budgets
DROP POLICY IF EXISTS "authenticated_insert_orcamentos_approval" ON public.orcamentos;
CREATE POLICY "authenticated_insert_orcamentos_approval" ON public.orcamentos
  FOR INSERT TO authenticated WITH CHECK (true);

-- RLS: allow authenticated users to update approval fields
DROP POLICY IF EXISTS "authenticated_update_orcamentos_approval" ON public.orcamentos;
CREATE POLICY "authenticated_update_orcamentos_approval" ON public.orcamentos
  FOR UPDATE TO authenticated USING (true) WITH CHECK (true);

-- RLS: allow authenticated users to select all budgets
DROP POLICY IF EXISTS "authenticated_select_orcamentos_approval" ON public.orcamentos;
CREATE POLICY "authenticated_select_orcamentos_approval" ON public.orcamentos
  FOR SELECT TO authenticated USING (true);

-- Ensure the anon policy for token-based reads still exists
...
-- Ensure enviar_orcamento_cliente RPC accepts enviado_cliente as valid starting status
-- (Already handled in 20260704160000_client_approval_workflow.sql, but re-asserting the guard)
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1 FROM public.orcamento_itens oi
    WHERE oi.orcamento_id = p_orcamento_id
      AND (coalesce(oi.quantidade, 0) <= 0 OR coalesce(oi.preco_unitario, 0) <= 0)
  ) THEN
  ...
  FROM public.projeto_itens WHERE orcamento_id = p_orcamento_id;

  SELECT count(*) INTO v_parcelas_existentes
  FROM public.projeto_parcelas WHERE orcamento_id = p_orcamento_id;

  SELECT count(*) INTO v_boletos_existentes
  FROM public.boletos WHERE orcamento_id = p_orcamento_id;
  ...
  IF v_itens_existentes > 0 OR v_parcelas_existentes > 0 OR v_boletos_existentes > 0 THEN
    RAISE EXCEPTION
      ...
      v_numero_orcamento, v_itens_existentes, v_parcelas_existentes, v_boletos_existentes
      USING ERRCODE = 'P0001';
  END IF;

  -- GUARD: Must be aprovado_cliente to proceed with financial approval
  IF v_orcamento.status != 'aprovado_cliente' THEN
    RAISE EXCEPTION 'Orçamento % não está no status "aprovado_cliente". Status atual: %',
      v_numero_orcamento, v_status_anterior
      USING ERRCODE = 'P0003';
  END IF;
  ...
  INSERT INTO public.projeto_itens (
    projeto_id, produto_id, descricao, quantidade, preco_unitario,
    desconto, subtotal, validado, orcamento_id
  )
  SELECT
    ...
    round(coalesce(oi.quantidade, 0) * coalesce(oi.preco_unitario, 0)
      * (1 - coalesce(oi.desconto, 0) / 100), 2),
    true,
    p_orcamento_id
  ...
  FOR v_i IN 1..v_qtd_parcelas LOOP
    ...
    INSERT INTO public.projeto_parcelas (
      projeto_id, numero_parcela, valor, data_fechamento,
      data_vencimento, status, forma_pagamento, orcamento_id, descricao
    )
    VALUES (
      v_orcamento.projeto_id, v_i, v_valor_parcela, current_date,
      v_data_base + v_prazos[v_i], 'pendente', v_orcamento.forma_pagamento,
      p_orcamento_id, 'Parcela gerada pelo orçamento ' || v_numero_orcamento
    );
    v_parcelas_criadas := v_parcelas_criadas + 1;
  END LOOP;

  INSERT INTO public.boletos (
    nosso_numero, numero_documento, parcela_id, orcamento_id,
    projeto_id, empresa_id, valor, vencimento, nome_pagador,
    status, tipo, venda, num_parcela, total_parcelas, emissao
  )
  SELECT
    'ORC-' || regexp_replace(v_numero_orcamento, '[^a-zA-Z0-9]', '', 'g') || '-P' || pp.numero_parcela,
    v_numero_orcamento || '-P' || pp.numero_parcela,
    pp.id, p_orcamento_id, v_orcamento.projeto_id, v_orcamento.empresa_id,
    pp.valor, pp.data_vencimento,
    coalesce(v_cliente_nome, 'Cliente do orçamento'),
    'pendente_registro', 'Nota Fiscal', NULL,
