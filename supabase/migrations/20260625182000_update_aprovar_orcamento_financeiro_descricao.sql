-- Fix aprovar_orcamento_financeiro: remove missing valor_total from projetos and ensure descricao is not null
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM public.orcamento_itens WHERE orcamento_id = p_orcamento_id LOOP
    IF NOT EXISTS (SELECT 1 FROM public.projeto_itens WHERE orcamento_id = p_orcamento_id AND produto_id = v_item.produto_id AND quantidade = v_item.quantidade) THEN
      ...
      -- Obter nome do produto caso a descrição do item seja nula
      IF v_item.descricao IS NULL OR trim(v_item.descricao) = '' THEN
        SELECT nome INTO v_produto_nome FROM public.produtos WHERE id = v_item.produto_id;
        v_descricao_final := COALESCE(v_produto_nome, 'Item sem descrição');
      ELSE
        v_descricao_final := v_item.descricao;
      END IF;
