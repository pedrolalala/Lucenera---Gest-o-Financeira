-- Fase 2: motor de status de orcamentos orientado a evento
-- Status deixa de ser editavel livremente e passa a ser resultado exclusivo de
-- acoes de negocio (enviar, aprovar cliente manual/token, aprovar financeiro,
-- editar apos aprovacao). Toda transicao continua registrada em
-- historico_status_orcamentos.

-- ==========================================================
-- 1. Nova coluna: origem da aprovacao do cliente (manual vs token)
--    Mantem status unificado (nao duplica 'aprovado' em dois valores) para nao
--    quebrar quem ja depende de status = 'aprovado'/'pendente_aprovacao_financeira'.
-- ==========================================================
ALTER TABLE public.orcamentos
  ADD COLUMN IF NOT EXISTS aprovado_cliente_origem text;

ALTER TABLE public.orcamentos
  DROP CONSTRAINT IF EXISTS orcamentos_aprovado_cliente_origem_check;
ALTER TABLE public.orcamentos
  ADD CONSTRAINT orcamentos_aprovado_cliente_origem_check
  CHECK (aprovado_cliente_origem IS NULL OR aprovado_cliente_origem IN ('manual', 'token'));

-- ==========================================================
-- 2. Normalizar valores de status legados para os canonicos.
--    Achado real em producao: a coluna usa rotulos em portugues
--    ('Aprovação Financeira', 'Orçamento Aprovado'), nao os valores
--    snake_case que o restante do codigo assume. Mapeamento decidido
--    linha a linha com o time (ver relatorio Grupo A/B/C):
--      - 'Aprovação Financeira'  -> pendente_aprovacao_financeira (3 orcamentos,
--        todos sem projeto_itens/parcelas/boletos ainda)
--      - 'Orçamento Aprovado'    -> aprovado (12 orcamentos; inclui casos com
--        projeto_itens ja gerados e casos historicos sem geracao retroativa,
--        por decisao explicita de nao reprocessar orcamentos antigos)
--    ORC-0013 mantem requer_revisao_financeira=true de proposito (contradicao
--    conhecida com projeto_itens ja existentes) para revisao manual futura;
--    apenas o rotulo textual do status e normalizado aqui, a flag nao e tocada.
-- ==========================================================
UPDATE public.orcamentos
SET status = 'pendente_aprovacao_financeira'
WHERE status = 'Aprovação Financeira';

UPDATE public.orcamentos
SET status = 'aprovado'
WHERE status = 'Orçamento Aprovado';

-- Nota: nao existe catch-all generico por requer_revisao_financeira=true aqui de
-- proposito. Um orcamento como ORC-0013 (status='Orçamento Aprovado' com artefatos
-- ja gerados, mas requer_revisao_financeira=true por inconsistencia historica) deve
-- virar 'aprovado' como os demais do mesmo rotulo, e a flag contraditoria fica para
-- revisao manual — nao deve ser reclassificado automaticamente como pendente.

-- ==========================================================
-- 3. Whitelist canonica de status (bloqueia qualquer valor livre digitado)
-- ==========================================================
ALTER TABLE public.orcamentos
...
-- ==========================================================
-- 4. aprovar_orcamento_cliente (interno, role-based) -> pendente_aprovacao_financeira
-- ==========================================================
...
BEGIN
  ...
  VALUES (p_orcamento_id, v_status_anterior, 'pendente_aprovacao_financeira', auth.uid()::text, 'Aprovação do cliente finalizada — aguardando aprovação financeira');

  RETURN jsonb_build_object(
    'orcamento_id', p_orcamento_id,
    'status_anterior', v_status_anterior,
    'status_novo', 'pendente_aprovacao_financeira',
    'sucesso', true
  );
END;
$$;
...
-- ==========================================================
-- 5. aprovar_orcamento_cliente_publico (token do cliente) -> pendente_aprovacao_financeira
-- ==========================================================
...
BEGIN
  ...
  INSERT INTO public.historico_status_orcamentos (orcamento_id, status_anterior, status_novo, usuario, observacao)
  VALUES (p_orcamento_id, v_status_anterior, 'pendente_aprovacao_financeira', null, 'Orçamento aprovado pelo cliente via link público — aguardando aprovação financeira');

  RETURN jsonb_build_object(
    'orcamento_id', p_orcamento_id,
    'status', 'pendente_aprovacao_financeira',
    'sucesso', true
  );
END;
$$;
...
-- ==========================================================
-- 6. aprovar_orcamento_cliente_manual (vendedor aprova em nome do cliente)
--    Endurecida por SPEC-ORC-05: valida frete, forma de pagamento e prazo
--    ANTES de mudar o status, nao so no passo financeiro.
-- ==========================================================
...
BEGIN
  ...
  INSERT INTO public.historico_status_orcamentos (orcamento_id, status_anterior, status_novo, usuario, observacao)
  VALUES (p_orcamento_id, v_status_anterior, 'pendente_aprovacao_financeira', auth.uid()::text, 'Aprovação manual do cliente pelo administrador — aguardando aprovação financeira');
...
-- ==========================================================
-- 7. aprovar_orcamento_financeiro agora exige pendente_aprovacao_financeira e
--    transiciona para 'aprovado' (status final, dispara integracao)
-- ==========================================================
...
BEGIN
  ...
  IF v_orcamento.status != 'pendente_aprovacao_financeira' THEN
    RAISE EXCEPTION 'Orçamento % não está aguardando aprovação financeira. Status atual: %', v_numero_orcamento, v_status_anterior
      USING ERRCODE = 'P0003';
  END IF;

  UPDATE public.orcamentos SET status = 'aprovado', requer_revisao_financeira = false WHERE id = p_orcamento_id;
...
-- ==========================================================
-- 8. Reset do ciclo de aprovacao ao editar cabecalho comercial de um orcamento
--    ja aprovado pelo cliente ou aprovado final (SPEC: editar apos aprovacao
--    reinicia aprovacao do cliente + financeira do zero)
-- ==========================================================
...
-- ==========================================================
-- 9. Mesmo reset quando os ITENS do orcamento mudam (insert/update/delete),
--    independente de qual RPC/caller tocou orcamento_itens.
-- ==========================================================
...
BEGIN
  ...
  IF v_status IN ('pendente_aprovacao_financeira', 'aprovado') THEN
    UPDATE public.orcamentos
...
DROP TRIGGER IF EXISTS trg_orcamento_item_change_reset ON public.orcamento_itens;
CREATE TRIGGER trg_orcamento_item_change_reset
  AFTER INSERT OR UPDATE OR DELETE ON public.orcamento_itens
  FOR EACH ROW EXECUTE FUNCTION public.handle_orcamento_item_change_reset();
