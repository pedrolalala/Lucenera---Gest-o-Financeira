-- Transition client approval to "Aprovação Financeira" status
-- Instead of skipping to 'aprovado', budgets now go through mandatory financial review
-- Updates: aprovar_orcamento_cliente, aprovar_orcamento_cliente_publico, aprovar_orcamento_cliente_manual
-- Also updates handle_orcamento_workflow trigger and aprovar_orcamento_financeiro input validation

-- ==========================================================
-- 1. Update aprovar_orcamento_cliente: transition to 'Aprovação Financeira'
-- ==========================================================
...
BEGIN
  ...
  IF v_orcamento.projeto_id IS NOT NULL THEN
    ...
    SET status = 'Aprovação Financeira'::projeto_status,
        updated_at = now()
...
-- ==========================================================
-- 2. Update aprovar_orcamento_cliente_publico: transition to 'Aprovação Financeira'
-- ==========================================================
...
-- ==========================================================
-- 3. Update aprovar_orcamento_cliente_manual: transition to 'Aprovação Financeira'
-- ==========================================================
...
-- ==========================================================
-- 4. Update handle_orcamento_workflow trigger
--    Handle 'Aprovação Financeira' for requer_revisao_financeira flag
-- ==========================================================
...
BEGIN
  ...
  -- set financial review flag and approval timestamp
...
-- ==========================================================
-- 5. Update aprovar_orcamento_financeiro: accept 'Aprovação Financeira' status
-- ==========================================================
...
BEGIN
  ...
  -- Accept 'Aprovação Financeira' as the primary valid status for financial processing
  IF v_orcamento.status NOT IN ('Aprovação Financeira', 'aprovado', 'aprovado_cliente') THEN
  ...
  -- Update budget status to aprovado_financeiro and clear financial review flag
  UPDATE public.orcamentos
