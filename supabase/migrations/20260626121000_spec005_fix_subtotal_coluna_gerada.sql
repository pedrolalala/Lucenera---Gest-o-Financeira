-- SPEC-005 — projeto_itens.subtotal passou a ser coluna GENERATED ALWAYS
-- (round(quantidade*preco_unitario*(1-desconto/100), 2), idêntica à fórmula
-- que a RPC calculava manualmente). Postgres não permite mais inserir valor
-- explícito nessa coluna. Remove subtotal do INSERT; o valor gerado é igual
-- ao que a RPC já calculava.
...
BEGIN
  ...
  -- subtotal é coluna GENERATED ALWAYS (round(quantidade*preco_unitario*
  -- (1-desconto/100),2)) — não inserir valor explícito, deixar o Postgres
  -- calcular.
