-- Fix: Remove subtotal from INSERT into projeto_itens
-- subtotal is a GENERATED ALWAYS column — cannot insert explicit values
-- Recreates aprovar_orcamento_financeiro to exclude subtotal from INSERT
...
BEGIN
  ...
  -- subtotal is a GENERATED ALWAYS column — excluded from INSERT so Postgres computes it automatically
  INSERT INTO public.projeto_itens (projeto_id, produto_id, descricao, quantidade, preco_unitario, desconto, validado, orcamento_id)
