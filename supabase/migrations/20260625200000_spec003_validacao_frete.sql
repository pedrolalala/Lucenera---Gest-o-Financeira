-- SPEC-003 — Validação e Coleta de Frete no Orçamento
--
-- orcamentos.frete_tipo e orcamentos.frete_valor já existem (SPEC-001), mas
-- nenhuma tela os preenche e a RPC de aprovação não os valida. Esta migration
-- adiciona as constraints de domínio/coerência e a validação na RPC.
--
-- Valores normalizados de frete_tipo: 'com_frete' | 'sem_frete'.
-- Hoje todos os orçamentos existentes têm frete_tipo NULL e frete_valor = 0,
-- então as constraints abaixo não quebram dados atuais.

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'orcamentos_frete_tipo_check'
  ) THEN
    ALTER TABLE public.orcamentos
      ADD CONSTRAINT orcamentos_frete_tipo_check
      CHECK (frete_tipo IS NULL OR frete_tipo IN ('com_frete', 'sem_frete'));
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'orcamentos_frete_coerencia_check'
  ) THEN
    ALTER TABLE public.orcamentos
      ADD CONSTRAINT orcamentos_frete_coerencia_check
      CHECK (
        frete_tipo IS NULL
        OR (frete_tipo = 'sem_frete' AND coalesce(frete_valor, 0) = 0)
        OR (frete_tipo = 'com_frete' AND frete_valor > 0)
      );
  END IF;
END $$;

-- Validação de frete em aprovar_orcamento_financeiro_base_spec001, antes do
-- UPDATE de status. Mantém o restante da função idêntico à última versão
-- (20260625120000_spec002_prazo_inicio_cobranca.sql).
