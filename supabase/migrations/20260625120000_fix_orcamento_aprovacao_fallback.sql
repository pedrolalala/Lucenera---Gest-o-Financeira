-- SPEC-001 — Fallback para aprovação de orçamento sem condições de pagamento
...
BEGIN
      ...
      BEGIN
        ...
        BEGIN
          ...
          VALUES (
            ...
            'aprovado',
            auth.uid()::text,
            'Aprovação integrada por aprovar_orcamento_financeiro_base_spec001'
          );
        EXCEPTION
  ...
  IF to_regprocedure('public.aprovar_orcamento_financeiro(uuid)') IS NOT NULL THEN
    EXECUTE $fn2$
      ...
      BEGIN
        IF to_regprocedure('public.aprovar_orcamento_financeiro_base_spec001(uuid)') IS NOT NULL THEN
          EXECUTE 'SELECT public.aprovar_orcamento_financeiro_base_spec001($1)'
            INTO v_result
            USING p_orcamento_id;
          ...
          RETURN coalesce(v_result, '{}'::jsonb) || jsonb_build_object('financeiro_usa_orcamento_id', true);
        END IF;
